...
1. Under **Manage**, select **Authentication (Preview)**.

1. Under **Redirect URI configurations**, select **+ Add Redirect URI**.

1. In **Select a platforms to add redirect URI**, select the tile for your application type (platform) to configure its settings.
...
1. Enter `https://localhost` for the Redirect URI.
...
1. Under **Manage**, select **Expose an API**, and then select **+ Add a scope**.

    ![An app registration's Expose an API pane in the Azure portal](./media/portal-02-expose-api.png)

1. You're prompted to set an **Application ID URI**. Set the value to **api://DemoAppAPI**
...
1. Next, specify the scope's attributes in the **Add a scope pane**. For this walk-through, use the values in the 3rd column - **Value**.
...
1. Make sure the State is set to **Enabled** then select **Add Scope**.
...
In this exercise, you registered an application in Microsoft Entra ID and exposed an API by defining application ID URI and scopes. This exercise showed how applications are represented in Microsoft Entra ID and how they expose permissions to clients.
...
1. In the left navigation, under **Entra ID**, select **Roles & admins**.

1. Select **+ New custom role** from the top menu.
...
1. In the New custom role dialog, on the Basics tab, in the name box, enter **My custom app role**.
...
In this exercise, you created a custom directory role with a focused set of credential-management permissions. This exercise showed how custom roles support fine-grained, least-privilege app governance.
